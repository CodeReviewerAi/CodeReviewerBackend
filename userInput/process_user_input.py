...
def process_user_input(input_function):
    ...
    def performKNNSearch(embedding, k=5, merge_threshold=-0.6, retry_attempts=3, retry_delay=5):
        ...
        for attempt in range(retry_attempts):
            try:
                ...
                print(f"Exception during KNN search: {e}. Retrying...")

            time.sleep(retry_delay)

        raise Exception("Failed to perform KNN search after multiple attempts.")
